...
class mTheme_Shortcode {
	...
	public function titles ($atts, $content){
		...
		$template = self::kopa_get_template_part ( 'titles/title', $atts['styles'] );
	...
	public function loop_items ($atts){
		...
		if ( in_array($atts['styles'], array('v2')) ) $values = vc_param_group_parse_atts($atts['values_v2']);
		
		ob_start();
		
		$template = self::kopa_get_template_part ( 'loop-items/loop-item', $atts['styles'] );
		if ( file_exists("{$template}") ) {
			require "{$template}";
		}
